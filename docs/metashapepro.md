...
- TOPs_MetashapePro_RunJob_Export_Colmap_Sequence_V001.hip
- TOPs_MetashapePro_RunJob_Static_V001.hip

To make full use of the "TOPs_MetashapePro_RunJob_Static_V001.hip" Houdini .hip file, you should add a "psx" folder to your Houdini project folder hierarchy. This is used to store the newly created Metashape .psx project files, and the report PDFs.
...
### MetashapePro_export_cameras_sequence.py

This script is used by the "TOPs\_MetashapePro\_RunJob\_Export\_Colmap\_Sequence\_V001.hip" example. It provides a Houdini TOPS driven approach to perform COLMAP camera pose sequence exports from an existing Metashape Pro .psx timeline sequence.

An "attribute create" parameter named "FRAME" holds the current timeline frame value "$F0". This frame value is passed to the Metashape Pro "chunk.frame" attribute which allows our Python script to access the Metashape Timeline frame number.
...
# Houdini timeline frame
frame = int(str(work_item.data.stringData("FRAME", 0)))
chunk.frame = frame
...
print("[Export Camera] Frame", str(frame))
